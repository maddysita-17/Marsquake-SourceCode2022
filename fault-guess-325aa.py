...
def eventoutput(depth,rank):
    ...
    if n == 0:
        data325ab.to_csv('./event-by-event/S0325aa/csvs/S0325aa_az123.csv', header=True)
    else:
        data325ab.to_csv('./event-by-event/S0325aa/csvs/S0325aa_az123.csv', mode='a', header=False)

    print('325aa - InSight Azimuth')
    Pp, Sp, Pa, Sa = eventbuild('325a', 38.4)
    ...
    if n == 0:
        data325ab.to_csv('./event-by-event/S0325aa/csvs/S0325aa_az57.csv', header=True)
    else:
        data325ab.to_csv('./event-by-event/S0325aa/csvs/S0325aa_az57.csv', mode='a', header=False)
    return
